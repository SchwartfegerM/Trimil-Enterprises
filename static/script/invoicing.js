...
xhr.onload = function() {
    if (this.status === 200) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            firstName.addEventListener('click', function () {let newFirstName = prompt('First Name: ', data.first_name); if(newFirstName != ""){firstName.innerHTML = newFirstName;data.first_name = newFirstName;fetch('/add/UPDATE/'+data[i].ID+"/First_Name/"+newFirstName, {method: 'POST'})}})
            ...
            lastName.addEventListener('click', function () {let newLastName = prompt('Last Name: ', data.Last_Name); if(newLastName != ""){ lastName.innerHTML = newLastName;data.Last_Name = newLastName;fetch('/add/UPDATE/'+data[i].ID+"/Last_Name/"+newLastName, {method: 'POST'})}})
            ...
            officeCharges.addEventListener('click', function () {let newOfficeCharges = prompt('Office Charges: ', data[i].office_charges); if(newOfficeCharges != ""){ officeCharges.innerHTML = newOfficeCharges;data.office_charges = newOfficeCharges;fetch('/add/UPDATE/'+data[i].ID+"/Office_Charges/"+newOfficeCharges, {method: 'POST'})}})
            ...
            projectManagement.addEventListener('click', function () {let newProjectManagement = prompt('Project Management: ', data.Project_Management); if(newProjectManagement != ""){ projectManagement.innerHTML = newProjectManagement;data.project_management = newProjectManagement;fetch('/add/UPDATE/'+data[i].ID+"/Project_Management/"+newProjectManagement, {method: 'POST'})}})
            ...
            travelCharges.addEventListener('click', function () {let newTravelCharges = prompt('Travel Charges: ', data.Travel_Charges); if(newTravelCharges != ""){ travelCharges.innerHTML = newTravelCharges;data.travel_charges = newTravelCharges;fetch('/add/UPDATE/'+data[i].ID+"/Travel_Charges/"+newTravelCharges, {method: 'POST'})}})
            ...
            consultancyCharges.addEventListener('click', function () {let newConsultancyCharges = prompt('Consultancy Charges: ', data.Consultancy_Charges); if(newConsultancyCharges != ""){ consultancyCharges.innerHTML = newConsultancyCharges;data.consultancy_charges = newConsultancyCharges;fetch('/add/UPDATE/'+data[i].ID+"/Consultancy_Charges/"+newConsultancyCharges, {method: 'POST'})}})
